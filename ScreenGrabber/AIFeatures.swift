...
class OCRManager: ObservableObject {
    ...
    /// Async version: extract text and confidence score from an image
    func extractTextWithConfidence(from image: NSImage) async throws -> (text: String, confidence: Double)? {
        guard let cgImage = image.cgImage(forProposedRect: nil, context: nil, hints: nil) else {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let request = VNRecognizeTextRequest { request, error in
                ...
                let averageConfidence = Double(totalConfidence / Float(observations.count))
